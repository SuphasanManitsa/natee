...
export async function POST(req) {
  ...
  try {
    ...
    if (request.image === '') {
      const temp = await db.execute('INSERT INTO product (p_id,p_name,p_storage,quantity) VALUES (?,?,?,?);',
        [p_id[0].p_id, request.formData.p_name, request.formData.p_storage, request.formData.quantity]);
    }
    else {
